...
<body>
  ...
  <script>
        ...
        th.addEventListener('blur', async function(e) {
          ...
          if (newName && newName !== originalDisplayName && !columns.some((c, i) => c === newName && i !== columns.indexOf(originalDisplayName))) {
            ...
            if (currentLogicalIndex !== -1) {
                ...
                // Send column rename to API - only send new_name parameter
                await sendColumnRenameToAPI(newName);
            }
          } else {
    ...
    async function sendColumnRenameToAPI(name) {
        const params = {
            new_name: name
        };
        const queryParams = new URLSearchParams(params).toString();
